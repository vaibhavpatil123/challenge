...
@Slf4j
@Repository
public class AccountsRepositoryInMemory
    implements com.db.awmd.challenge.repository.AccountsRepository {
  ...
  public AccountsRepositoryInMemory(
      AppEnvParameters applicationParameters, NotificationService notificationService) {
  ...
  private Account checkIsAccountValid(String accountNo) {
    ...
    log.info(
        "com.db.awmd.challenge.repository.impl.AccountsRepositoryInMemory.checkIsAccountValid Start");
    ...
    if (account == null) {
      ...
      throw new AccountAccessException(
          APPTransactionMessages.ACCOUNT_NOT_AVAILABLE.getMessage(),
          APPTransactionMessages.ACCOUNT_NOT_AVAILABLE.getErrorCode(),
          null);
    }

    log.info(
        "com.db.awmd.challenge.repository.impl.AccountsRepositoryInMemory.checkIsAccountValid done");
  ...
  private void checkIsSameAccounts(String accountNo, String toAccountNo) {
    log.info(
        "com.db.awmd.challenge.repository.impl.AccountsRepositoryInMemory.checkIsSameAccounts Start");
  ...
  public synchronized void clearAccounts() {
    try {
      ...
      throw new AccountTransactionException(
          APPTransactionMessages.SYSTEM_ERROR.getMessage(),
  ...
  public void create(Account account) {
    ...
    if (previousAccount != null) {
      ...
      throw new AccountCreateException(
          APPTransactionMessages.ACCOUNT_DUPLICATE.getMessage(),
  ...
  private Boolean executeTransaction(
      final Account account, final BigDecimal amount, final TransactionType transactionType) {
  ...
  private void handelTransaction(
      BigDecimal amount, Account fromAccount, Account toAccount, String transactionId) {
    if (this.executeTransaction(fromAccount, amount, TransactionType.DEBIT).
      ...
      throw new AccountTransactionException(
          APPTransactionMessages.TRANSACTION_FAILED_TO_ROLLBACK.getMessage(),
    ...
    log.error(
        "com.db.awmd.challenge.repository.impl.AccountsRepositoryInMemory.handelTransaction failed executeTransaction DEBIT_FAIL");
  }
  ...
  private void sendNotifications(
    ...
    this.notificationService.notifyAboutTransfer(
        new Notification(fromAccount, amount, transactionId, debitSuccess));
  }
  ...
  private void lockAndExecuteAction(
      BigDecimal amount, String transactionId, Account fromAccount, Account toAccount) {
    try {
      ...
      log.error(
          "AccountsRepositoryInMemory :: transferMoney InterruptedException due to {0} ",
          e.getMessage());
      throw new AccountTransactionException(
          APPTransactionMessages.SYSTEM_ERROR.getMessage(),
          APPTransactionMessages.SYSTEM_ERROR.getErrorCode(),
  ...
  private void validateAccountCreation(Account account) {
    if (account
            .getBalance()
            .compareTo(new BigDecimal(this.appEnvParamters.getAccountMinBalanceLimit()))
        < 0) {
      log.error(
  ...
  private void validateAmount(Account account, BigDecimal amount) {
    if (account.getBalance().compareTo(amount) <= 0) {
      throw new AccountAccessException(
          APPTransactionMessages.ACCOUNT_TRANSACTION_LOW_BALANCE.getMessage(),
